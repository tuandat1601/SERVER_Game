/*
  ...
  - You are about to alter the column `createdAt` on the `game_backend_user` table. The data in that column could be lost. The data in that column will be cast from `DateTime(0)` to `DateTime`.
  ...
  - Added the required column `gameId` to the `game_code_used` table without a default value. This is not possible if the table is not empty.
  - Added the required column `gameId` to the `game_comcode_used` table without a default value. This is not possible if the table is not empty.

*/
-- DropIndex
DROP INDEX `game_code_used_code_idx` ON `game_code_used`;

-- AlterTable
ALTER TABLE `game_backend_user` MODIFY `createdAt` DATETIME NOT NULL DEFAULT NOW(),
...
-- AlterTable
ALTER TABLE `game_code_used` ADD COLUMN `gameId` SMALLINT UNSIGNED NOT NULL;

-- AlterTable
ALTER TABLE `game_comcode_used` ADD COLUMN `gameId` SMALLINT UNSIGNED NOT NULL;
...
-- AlterTable
ALTER TABLE `orders` MODIFY `createdAt` DATETIME NOT NULL DEFAULT NOW(),
    MODIFY `updatedAt` TIMESTAMP(0) NOT NULL DEFAULT NOW() ON UPDATE NOW();
...
-- CreateIndex
CREATE INDEX `game_code_used_code_gameId_idx` ON `game_code_used`(`code`, `gameId`);
